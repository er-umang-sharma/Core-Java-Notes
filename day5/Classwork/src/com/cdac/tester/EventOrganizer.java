                ...
                try {
                    ...
                    switch (sc.nextInt()) {
                        case 1 -> {
                            ...
                            Faculty f = new Faculty(sc.next(), sc.next(), sc.nextInt(), sc.nextLine());
                            for (i=0; i<lastSeatBooked; i++) {
                                if (totalSeatsAvailable[i].equals(f)) {
                                    System.out.println("Sorry, the user with the same first name and last name is already registered.");
                                    alreadyPresent=true;
                                    break;
                                }
                            }
                            if (alreadyPresent) break;
                            totalSeatsAvailable[lastSeatBooked++] = f;
                        ...
                        case 2 -> {
                            ...
                            Student s = new Student(sc.next(), sc.next(), sc.nextInt(), sc.nextLine());
                            for (i=0; i<lastSeatBooked; i++) {
                                if (totalSeatsAvailable[i].equals(s)) {
                                    System.out.println("Sorry, the user with the same first name and last name is already registered.");
                                    alreadyPresent=true;
                                    break;
                                }
                            ...
                            if (alreadyPresent) break;
                            totalSeatsAvailable[lastSeatBooked++] = s;
                            System.out.println("Registration successful. Seat: " + lastSeatBooked + ". The data entered is:");
                            System.out.println(totalSeatsAvailable[lastSeatBooked - 1]);
                        }
                        case 3 -> {
